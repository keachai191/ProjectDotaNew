                ...
                <div class="modal-content">
                    <form action="RequestStore" method="post" enctype="multipart/form-data">
                            ...
                            @if(!empty (session()->get('checkday')))
                                ...
                                <div class="alert alert-danger">
                                    <center><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> <strong>ข้อผิดพลาด!</strong>
                                        ...
                                        {{session()->get('morn')}}  {{session()->get('after')}}  {{session()->get('even')}}
                                        <br/>
                        ...
                        <div class="modal-body">
                            ...
                            @if(session()->get('UserPrimary')== null)
                                <center><h3>คำขอได้ถูกดำเนินการแล้ว</h3><br/><br/>
                                    <a href="viewreques" type="button" class="btn btn-primary" aria-label="Left Align">
                                        <span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></span>
                                        ดูประวัติคำร้องขอ
                                    </a></center>

                            @elseif(!session()->get('UserPrimary')== null)
                                ...
                                <div><label>ถึงช่างภาพ : {{session()->get('UserName')}}</label><br/>

                                    <label>รหัสสมาชิก : #P0{{session()->get('UserPrimary')}}</label></div>

                                @if(session()->get('data') == null)
                                    <label style="align-content: inherit">วันที่ : <input type="date"
                                ...
                                <div style="align-content: inherit">
                                    ...
                                    @if(session()->get('helf1') == 0)
                                        <label> ช่วงเวลา :
                                            <select name="verify" id="prov2">
                                                <option value="ช่วงเช้า" selected="selected">ช่วงเช้า</option>
                                    ...
                                    @elseif(!session()->get('helf1') == null or !session()->get('helf') == null)

                                        <label>
                                            ช่วงเวลา :
                                            @if(session()->get('helf') == 2)
                                                <input type="checkbox"
                                                       checked disabled>เต็มวัน
                                            ...
                                            @elseif(session()->get('helf1') == 1)
                                                <input type="checkbox"
                                                       checked disabled>ช่วงเช้า
                                                <input name="morning" type="hidden" value="ช่วงเช้า">

                                            @elseif(session()->get('helf1') == 2)
                                                <input type="checkbox"
                                                       checked disabled>ช่วงบ่าย
                                                <input name="afternoon" type="hidden" value="ช่วงบ่าย">

                                            @elseif(session()->get('helf1') == 3)
                                                <input type="checkbox"
                                                       checked disabled>ช่วงเย็น
                                                <input name="evening" type="hidden" value="ช่วงเย็น">
                                            @endif
                                ...
                                <div class="col-md-5 col-md-offset5"><textarea style=" border: 2px solid #3498db"
                                                                               class="form-control"
                                                                               name="detail_request"
                                                                               rows="3"
                                                                               cols="40"
